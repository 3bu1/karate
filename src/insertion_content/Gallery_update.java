...
/**
 * Servlet implementation class Gallery_update
 */
@WebServlet("/Gallery_update")
public class Gallery_update extends HttpServlet {
	DataBase db;
	int i=0;
	 ...
	 HttpSession sess=request.getSession();
		PrintWriter out = response.getWriter();
		  ...
		  if (item.isFormField()){
			  ...
			  fieldvalue=fieldvalue.concat(item.getString()+",,,");
			  String[] array_image = fieldvalue.split(",,,");
			//  out
...
//		  		int rs = pst.executeUpdate();
	...
	try{
		...
		//String csx= request.getParameter("dummy_title");
		
           	sql = "UPDATE "+request.getParameter("kskdata")+" SET image ='"+img2+"',imageinfo='"+img+"' WHERE imageinfo='"+imghide+"'";
           out.println(imghide);
           	sess.setAttribute("kanka_content_update", "update_entry");
